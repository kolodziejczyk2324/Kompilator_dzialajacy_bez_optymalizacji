...
//to dla inc
#define FOR_4 create_number(code, iteratory[my_toUpperCase(stack_iteratory.daj_top())].store, 0);\
	STORE(1)\
	POP_AND_WRITE_ETYK\
	ZERO(0)\
	STORE(4)\
...
//to dla dec
#define DOWNTO_FOR_4 create_number(code, iteratory[my_toUpperCase(stack_iteratory.daj_top())].store, 0);\
	STORE(1)\
	POP_AND_WRITE_ETYK\
	create_number(code, iteratory[my_toUpperCase(stack_iteratory.daj_top())].store, 0);\
	LOAD(1)\
...
#line 287 "parser.tab.c" /* yacc.c:339  */
...
#line 370 "parser.tab.c" /* yacc.c:355  */
};
...
{
       0,   240,   240,   243,   244,   247,   253,   254,   256,   260,
     262,   265,   265,   275,   277,   279,   275,   284,   287,   284,
     295,   294,   300,   299,   304,   306,   306,   311,   311,   316,
     316,   321,   321,   327,   328,   332,   333,   336,   339,   345,
     346,   347,   348,   352,   353,   354,   355,   359,   360,   361,
     362,   366,   367,   368,   369,   380,   381,   382,   383,   388,
     389,   390,   391,   396,   397,   398,   399,   404,   406,   408,
     410,   416,   418,   420,   422,   431,   436,   441,   446,   453,
     472,   491
};
#endif
...
#line 472 "parser.y" /* yacc.c:1646  */
    {	check_identifier((yyvsp[-3].sval), 1);
							if(jestIterator() && iteratory.find((yyvsp[-1].sval)) != iteratory.end()){ //sprawdz czy dostales iterator jako identyfikator
								if((yyvsp[-1].sval)==stack_iteratory.daj_top()){ //jesli iterator jest tym w ktorym aktualnie jestesmy
									create_number(code, 0, 0); //tutaj bedziemy zapisywac nasz iterator (jego wartosc)
									STORE(4) //zapisujemy go do miejsca w pamieci 0
								}
								else{ //jesli iterator nie jest tym aktualnym z petli to jest on gdzies w pamieci, traktujemy go jak zmienna
									create_number(code, iteratory[(yyvsp[-1].sval)].store, 0); //zapisz w pamieci 0 adres tego iteratora
								}
							}
							else{
